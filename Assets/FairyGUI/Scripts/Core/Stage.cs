...
#pragma warning restore 0649
            ...
            : base()
        {
            ...
            if (Application.platform == RuntimePlatform.WindowsPlayer
                ...
                touchScreen = Touchscreen.current.enabled && SystemInfo.deviceType != DeviceType.Desktop;
        ...
        void GetHitTarget()
        {
            ...
            {
                _touchTarget = null;
                for (int i = 0; i < Touchscreen.current.primaryTouch.tapCount.ReadValue(); ++i)
                {
                    var uTouch = Touchscreen.current.primaryTouch;

                    Vector2 pos = uTouch.position.ReadValue();
                    pos.y = _contentRect.height - pos.y;
                    ...
                    for (int j = 0; j < 5; j++)
                    {
                        if (_touches[j].touchId == uTouch.touchId.ReadValue())
                        {
                            touch = _touches[j];
                            break;
                        }
                    ...
                    {
                        touch = free;
                        if (touch == null || uTouch.phase.ReadValue() != UnityEngine.InputSystem.TouchPhase.Began)
                            continue;

                        touch.touchId = uTouch.touchId.ReadValue();
                    }

                    if (uTouch.phase.ReadValue() != UnityEngine.InputSystem.TouchPhase.Stationary)
        ...
        internal void HandleGUIEvents(Event evt)
        {
            ...
            {
                if (_IMEComposite && Keyboard.current.imeSelected.isPressed)
                {
                    _IMEComposite = false;
        ...
        void UpdateTouchPosition()
        {
            if (_frameGotTouchPosition != Time.frameCount)
            {
                ...
                else if (touchScreen)
                {
                    ...
                    {
                        var uTouch = Touchscreen.current.primaryTouch;
                        _touchPosition = uTouch.position.ReadValue();
                        _touchPosition.y = _contentRect.height - _touchPosition.y;
                    }
                }
                else
        ...
        {
            _IMEComposite = Keyboard.current.imeSelected.isPressed;
        ...
        void HandleMouseEvents()
        {
            ...
            if (Mouse.current.leftButton.isPressed || Mouse.current.rightButton.isPressed || Mouse.current.middleButton.isPressed)
            {
                if (!touch.began)
                {
                    _touchCount = 1;
                    touch.Begin();
                    touch.button = Mouse.current.rightButton.isPressed ? 2 : (Mouse.current.middleButton.isPressed ? 1 : 0);
                    SetFous(touch.target);
            ...
            if (Mouse.current.leftButton.wasReleasedThisFrame || Mouse.current.rightButton.wasReleasedThisFrame || Mouse.current.middleButton.wasReleasedThisFrame)
            {
                if (touch.began)
                {
                    _touchCount = 0;
            ...
            if (Mouse.current.leftButton.wasReleasedThisFrame && _currentCursor != null)
                _ChangeCursor(_currentCursor);
        }

        void HandleTouchEvents()
        {
            int tc = Touchscreen.current.primaryTouch.tapCount.ReadValue();
            for (int i = 0; i < tc; ++i)
            {
                var uTouch = Touchscreen.current.touches[i];
                ...
                if (uTouch.phase.ReadValue() == UnityEngine.InputSystem.TouchPhase.Began)
                {
                    if (!touch.began)
                    {
                        _touchCount++;
                ...
                else if (uTouch.phase.ReadValue() == UnityEngine.InputSystem.TouchPhase.Canceled || uTouch.phase.ReadValue() == UnityEngine.InputSystem.TouchPhase.Ended)
                {
                    if (touch.began)
                    {
                        _touchCount--;
                        ...
                        if (uTouch.phase.ReadValue() != UnityEngine.InputSystem.TouchPhase.Canceled)
                        {
                            DisplayObject clickTarget = touch.ClickTest();
                            if (clickTarget != null)
                            {
                                touch.clickCount = uTouch.tapCount.ReadValue();
                                touch.UpdateEvent();
                                clickTarget.BubbleEvent("onClick", touch.evt);
                            }
                        }
