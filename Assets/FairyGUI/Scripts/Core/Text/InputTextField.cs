...
//            Keyboard.current.onIMECompositionChange += (str) =>
//            {
//                string str1 = str.ToString();
//            };
            _composing = GameInit.imeString.Length;
            
            if (_composing > 0)
            {
                StringBuilder buffer = new StringBuilder();
                GetPartialText(0, _caretPosition, buffer);
                buffer.Append(GameInit.imeString);
        ...
        void UpdateCaret(bool forceUpdate = false)
        {
            ...
            if (_editing)
                cp = GetCharPosition(_caretPosition + GameInit.imeString.Length);
            else
        ...
        void UpdateSelection(TextField.CharPosition cp)
        {
            ...
            if (_editing && GameInit.imeString.Length > 0)
            {
                if (_selectionStart < _caretPosition)
                {
                    cp = GetCharPosition(_caretPosition);
                    ...
                    start = GetCharPosition(_selectionStart + GameInit.imeString.Length);
            }
            else
                start = GetCharPosition(_selectionStart);
...
//                Input.imeCompositionMode = IMECompositionMode.Auto;
        ...
        bool HandleKey(InputEvent evt)
        {
            ...
            {
                if (GameInit.imeString.Length > 0 && _editable)
                {
        ...
        {
            if (_composing != 0 && GameInit.imeString.Length == 0)
                UpdateText();
        }
