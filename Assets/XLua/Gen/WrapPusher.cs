...
namespace XLua
{
    public partial class ObjectTranslator
    {
        ...
        class IniterAdderUnityEngineVector2
        {
			...
			static void Init(LuaEnv luaenv, ObjectTranslator translator)
			{
				...
				translator.RegisterPushAndGetAndUpdate<Lib.KeyCode>(translator.PushLibKeyCode, translator.Get, translator.UpdateLibKeyCode);
        ...
        int LibKeyCode_TypeID = -1;
		int LibKeyCode_EnumRef = -1;
        
        public void PushLibKeyCode(RealStatePtr L, Lib.KeyCode val)
        {
            if (LibKeyCode_TypeID == -1)
            {
			    bool is_first;
                LibKeyCode_TypeID = getTypeId(L, typeof(Lib.KeyCode), out is_first);
				
				if (LibKeyCode_EnumRef == -1)
				{
				    Utils.LoadCSTable(L, typeof(Lib.KeyCode));
				    LibKeyCode_EnumRef = LuaAPI.luaL_ref(L, LuaIndexes.LUA_REGISTRYINDEX);
				}
				
            }
			
			if (LuaAPI.xlua_tryget_cachedud(L, (int)val, LibKeyCode_EnumRef) == 1)
            {
			    return;
			}
			
            IntPtr buff = LuaAPI.xlua_pushstruct(L, 4, LibKeyCode_TypeID);
            if (!CopyByValue.Pack(buff, 0, (int)val))
            {
                throw new Exception("pack fail fail for Lib.KeyCode ,value="+val);
            }
			
			LuaAPI.lua_getref(L, LibKeyCode_EnumRef);
			LuaAPI.lua_pushvalue(L, -2);
			LuaAPI.xlua_rawseti(L, -2, (int)val);
			LuaAPI.lua_pop(L, 1);
        ...
        public void Get(RealStatePtr L, int index, out Lib.KeyCode val)
        {
		    LuaTypes type = LuaAPI.lua_type(L, index);
            if (type == LuaTypes.LUA_TUSERDATA )
            {
			    if (LuaAPI.xlua_gettypeid(L, index) != LibKeyCode_TypeID)
				{
				    throw new Exception("invalid userdata for Lib.KeyCode");
				}
                ...
                if (!CopyByValue.UnPack(buff, 0, out e))
                {
                    throw new Exception("unpack fail for Lib.KeyCode");
                }
				val = (Lib.KeyCode)e;
            ...
            {
                val = (Lib.KeyCode)objectCasters.GetCaster(typeof(Lib.KeyCode))(L, index, null);
            }
        }
		
        public void UpdateLibKeyCode(RealStatePtr L, int index, Lib.KeyCode val)
        {
            ...
            if (LuaAPI.lua_type(L, index) == LuaTypes.LUA_TUSERDATA)
            {
                ...
                if (!CopyByValue.Pack(buff, 0,  (int)val))
                {
                    throw new Exception("pack fail for Lib.KeyCode ,value="+val);
                }
            }
	...
	public partial class StaticLuaCallbacks
    {
	    internal static bool __tryArrayGet(Type type, RealStatePtr L, ObjectTranslator translator, object obj, int index)
		{
			...
			else if (type == typeof(Lib.KeyCode[]))
			{
			    Lib.KeyCode[] array = obj as Lib.KeyCode[];
				translator.PushLibKeyCode(L, array[index]);
				return true;
			}
